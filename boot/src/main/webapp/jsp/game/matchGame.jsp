...
<head>
    <meta charset="UTF-8">
    <title>래드몼 매칭 가위바위보</title>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/sockjs-client/1.5.1/sockjs.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/stomp.js/2.3.3/stomp.min.js"></script>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Noto+Serif+KR:wght@500&display=swap');
        body { font-family: 'Noto Serif KR', serif; background: url('https://www.transparenttextures.com/patterns/asfalt-light.png'), linear-gradient(to bottom, #fef6e4, #f0e3ca); background-blend-mode: overlay; text-align: center; padding: 30px; margin: 0; color: #3b2f2f; }
        h2 { font-size: 28px; color: #4e3d30; background-color: #fef4dc; display: inline-block; padding: 10px 20px; border-radius: 12px; border: 3px double #b99b6b; box-shadow: 0 0 10px rgba(100, 80, 40, 0.15); }
        button { padding: 12px 25px; font-size: 18px; margin: 10px; border: 2px solid #a88756; border-radius: 10px; background-color: #fff3c4; color: #3b2f2f; cursor: pointer; transition: background-color 0.2s; box-shadow: 2px 2px 6px rgba(0, 0, 0, 0.1); }
        button:hover:not(:disabled) { background-color: #f2d694; }
        button:disabled { background-color: #ddd2b2; cursor: not-allowed; }
        .box { background: #fffbe6; padding: 25px; margin: 10px; border-radius: 12px; border: 2px solid #ccb37d; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.08); min-width: 220px; }
        .game-section { display: flex; justify-content: center; align-items: center; gap: 60px; margin-top: 30px; }
        #status, #result { font-size: 22px; margin-top: 20px; font-weight: bold; color: #4e3d30; }
        #endEmoji { font-size: 100px; margin-top: 30px; display: none; }
        .move-buttons { margin-top: 25px; }
        .move-buttons button { background-color: #fff; color: #112d4e; border: 2px solid #3f72af; }
        #roundPrompt, #finalMessage { position: fixed; top: 40%; left: 50%; transform: translate(-50%, -50%); font-size: 48px; font-weight: bold; padding: 30px 50px; border-radius: 20px; z-index: 9999; display: none; animation: fadeInOut 2s ease-in-out; }
        #roundPrompt { color: #8b5e3c; background-color: rgba(255, 248, 220, 0.95); border: 4px double #a88756; }
        #finalMessage { font-size: 60px; color: #fff; }
        @keyframes fadeInOut {
